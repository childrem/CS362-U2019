...
// choice1 = treasure card that the player is trashing
// choice2 = treasure card that the player is gaining
void mine_refactor(int choice1, int choice2, struct gameState *state, int handPos) {

	int currentPlayer = whoseTurn(state);					// Variable to keep track of the current player
	int i = 0;												// For loop counter 
	int choice1Card = state->hand[currentPlayer][choice1];	// Remember the choice1 card that player wants to trash
	...
	// Gain the choice2 card into the player's hand
...
void tribute_refactor(struct gameState *state, int handPos) {
	
	int currentPlayer = whoseTurn(state);					// Variable to keep track of the current player
	int nextPlayer = currentPlayer + 1;						// Variable to keep track of the next player
	if (nextPlayer > (state->numPlayers - 1)) {
		nextPlayer = 0;
	}
	int tributeRevealedCards[2] = {-1, -1};					// To keep track of the tributed cards
	int numTributeCards = 0;								// To keep track of how many tributed cards we have in case we have duplicates
	int i = 0;												// For loop counter
	
	// If the next player has no cards to tribute
	...
	// If the next player has 1 card between the discard pile and deck
	else if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) == 1){
		
		// If the next player's deck has more than 0 cards
		if (state->deckCount[nextPlayer] > 0){
			
			// Set the first tribute card to the next player's deck card then discard the card
		...
		// Else the player has one card in their discard pile instead
		else {
			
			// Set the first tribute card to the next player's discard card
	...
	// Else the next player has 2 or more cards between their deck and discard pile
		...
		if (state->deckCount[nextPlayer] == 0){
			...
			// Move all cards from discard pile into deck
			...
			// Set the first tribute card and discard the card from the deck
			tributeRevealedCards[0] = state->deck[nextPlayer][state->deckCount[nextPlayer]-1];
			state->discard[nextPlayer][state->discardCount[nextPlayer]] = state->deck[nextPlayer][state->deckCount[nextPlayer]-1];
			state->discardCount[nextPlayer]++;
			state->deckCount[nextPlayer]--;
			
			// Set the second tribute card and discard the card from the deck
		...
		// Else if the next player has only one card in their deck
		else if (state->deckCount[nextPlayer] == 1){
			
			// Set the card in the deck as first tribute card
			tributeRevealedCards[0] = state->deck[nextPlayer][state->deckCount[nextPlayer]-1];
			state->discard[nextPlayer][state->discardCount[nextPlayer]] = state->deck[nextPlayer][state->deckCount[nextPlayer]-1];
			state->discardCount[nextPlayer]++;
			state->deckCount[nextPlayer]--;
		...
		// Else the next player has 2 or more cards in their deck
		else {
		...
		// If we have a duplicate card, drop one and set the number of tributed cards to 1
	...
	// Discard tribute card from hand as played
...
// choice1 = position of card that player chose to return
// choice2 = number of cards that the player is opting to return
void ambassador_refactor(int choice1, int choice2, struct gameState *state, int handPos) {
	...
	// If the player choices the Ambassador card as the card to return, return from the function
	if (choice1 == handPos) {
		return;
	}
	...
	// If the number of cards that can be returned is less than how many the player wants to return, return from the function
	if (returnCards < choice2) {
		return;
	}
	
	// increase supply count for chosen card by amount being discarded
    state->supplyCount[choice1Card] += choice2;
	
	// Iterate until we've returned the amount of cards the player opted to return
    for (i = 0; i < choice2; i++) {
		
		// Iterate until we have no more cards in the player's hand
		for (j = 0; j < state->handCount[currentPlayer]; j++) {
			
			// If a card matches what the player wants to return, trash it and break out of the inner for loop
			if (state->hand[currentPlayer][j] == choice1Card) {
				discardCard(j, currentPlayer, state, 1);
				break;
			}
	    }
	}
	...
	// Discard the ambassador card as played
	for (i = 0; i < state->handCount[currentPlayer]; i++) {
...
void minion_refactor(int choice1, struct gameState *state, int handPos) {
	...
	if(choice1 > 0) {
		...
		// Discard Minion card from hand as played
		discardCard(handPos, currentPlayer, state, 0);
	}
	...
	// Else discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
		...
		while(numHandCards(state) > 0) {
			state->discard[currentPlayer][state->discardCount[currentPlayer]] = state->hand[currentPlayer][0]; 
			state->discardCount[currentPlayer]++;
			discardCard(0, currentPlayer, state, 1);
		}
		
		// current player draws four cards
		for (i = 0; i < 4; i++) {
			drawCard(currentPlayer, state);
		}
		
		// each other player with at least 5 cards in their hand discards their hand and draws 4 cards
		for (i = 0; i < state->numPlayers; i++) {
			
			// if the current selection isn't the current player
			if (i != currentPlayer) {
				...
				if ( state->handCount[currentPlayer] > 4 ) {			// BUG HERE!!! This should be state->handCount[i] > 4
					...
					while( state->handCount[i] > 0 ) {
						state->discard[i][state->discardCount[i]] = state->hand[i][0]; 
						state->discardCount[i]++;
						discardCard(0, i, state, 1);
					}
				
					// that player draws four cards
					for (j = 0; j < 4; j++) {
						drawCard(i, state);
					}
				}
...
void baron_refactor(int choice1, struct gameState *state, int handPos) {
	
	int currentPlayer = whoseTurn(state);					// Variable to keep track of the current player
	int playedCard = state->hand[currentPlayer][handPos];	// To keep track of the baron card to discard after
	...
	if (choice1 != 0) {
		...
		// Check through each card in player's hand
		for (i = 0; i < state->handCount[currentPlayer]; i++) {
	...
	// Else, the player has no estates. Check supply of Estates, if there is more than 0, gain one
	if (supplyCount(estate, state) > 0){
						
		gainCard(estate, state, 0, currentPlayer);		// Gain estate card
		state->supplyCount[estate]--;					// Decrement estates supply
		
		// If there are no estate cards in the supply pile afterwards, check to see if the game is over
		if (supplyCount(estate, state) == 0){
			isGameOver(state);
		}
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:
		...
		mine_refactor(choice1, choice2, state, handPos);
    ...
    case ambassador:		
		...
		ambassador_refactor(choice1, choice2, state, handPos);
